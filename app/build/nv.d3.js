...
nv.models.scatter = function(messageTarget) {
    "use strict";
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            function pushData(data){
                ...
                request.jsonrpc = "2.0";
                request.params = {};
                request.method = 'pushData';

                request.params.uid = "mmm";
                ...
                function displaySearchResult(response) {
                    if (response.error)
                        ...
                        function displayBox(data, colorcode, updateTarget){
                            ...
                            var leftOffset = jQuery(".nv-axisMin-x").position().left;
                            ...
                            var rightOffset = jQuery(".nv-axisMax-x").position().left;

                            // var minDateMarkWidth =

                            var offsetStart = leftOffset+jQuery(".nv-axisMin-x").children().width()/2;
                            var offsetEnd = rightOffset+jQuery(".nv-axisMax-x").children().width()/2;
                            ...
                            var stressDateStr = '';
                             if(updateTarget===1)
                            {
                                updateTargetStr = 'stressMessage';
                                var stressDateStr = "The user has a higher stress level on ";
                            }
                            else if(updateTarget===2)
                            {
                                updateTargetStr = 'notStressMessage';
                                var stressDateStr = "The user has a lower stress level on ";
                            }
                            jQuery.each(result, function(i, coord){
                            ...
                            .attr("d", rightRoundedRect(coord[0], coord[1], coord[2], coord[3], 10));

                            var clusterCenter = coord[0]+coord[2]/2 + offsetStart;
                            ...
                            var timeMillsStart =
                            (coord[0] - offsetStart)/(offsetEnd - offsetStart)*(dateMillsMax - dateMillsMin)
                            + dateMillsMin;

                             var timeMillsEnd =
                            (coord[0]+coord[2] - offsetStart)/(offsetEnd - offsetStart)*(dateMillsMax - dateMillsMin)
                            + dateMillsMin;

                            var timeStampStart = timeFormatToShow(new Date(timeMillsStart));
                            var timeStampEnd = timeFormatToShow(new Date(timeMillsEnd));


                            console.log("stressDateStr: "+  stressDateStr);

                            stressDateStr = stressDateStr+",  (from "+timeStampStart+" to "+timeStampEnd+")";
            ...
            points.enter().append('path')
                ...
                .attr('transform', function(d) {
                    addOnCount++;
                    // console.log("d[0] "+nv.utils.NaNtoZero(x0(getX(d[0],d[1])))+", d[1] "+nv.utils.NaNtoZero(y0(getY(d[0],d[1]))));

                    var x = nv.utils.NaNtoZero(x0(getX(d[0],d[1])));
                    var y = nv.utils.NaNtoZero(y0(getY(d[0],d[1])));
                    if(y<min){
                        min = y;
                    }
                    if(y>max){
            ...
            var pointsStress = [];
            var bar = max- (max-min)/4;
            jQuery.each(pointsToSend, function(i, val){
                if(val[1]> bar)
                {
                    pointsStress.push(val);
                }
            });


            var pointNotStress = [];
            var barNotStress = max- 3*(max-min)/4;
            jQuery.each(pointsToSend, function(i, val){
                if(val[1] < barNotStress)
                {
                    pointNotStress.push(val);
                }
            });

            var dataToSend = {}
            dataToSend.stressed = pointsStress;
            dataToSend.notStressed = pointNotStress;
...
nv.models.scatterChart = function(messageTarget) {
    "use strict";
    ...
    var scatter      = nv.models.scatter(messageTarget)
        , xAxis        = nv.models.axis()
        , yAxis        = nv.models.axis()
        , legend       = nv.models.legend()
